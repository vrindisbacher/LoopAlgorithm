...
public final class StandardRetrospectiveCorrection: RetrospectiveCorrection {
    ...
    public func computeEffect(
        ...
        totalGlucoseCorrectionEffect = LoopQuantity(
            unit: unit, doubleValue: currentDiscrepancyValue)

        let retrospectionTimeInterval = currentDiscrepancy.endDate.timeIntervalSince(
            currentDiscrepancy.startDate)
        let discrepancyTime = max(
            retrospectionTimeInterval, retrospectiveCorrectionGroupingInterval)
        let velocity = LoopQuantity(
            unit: .milligramsPerDeciliterPerSecond,
            doubleValue: currentDiscrepancyValue / discrepancyTime)
